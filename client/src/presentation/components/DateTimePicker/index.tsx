...
export const DateTimePicker = ({
    ...
    const dislayFormat = useMemo(() => {
        const displayPlaceHolder = placeHolder ? placeHolder : 'Pick a date';
        if (!displayValue) return displayPlaceHolder;
        return format(
            displayValue,
                    ...
                    <div className="relative overflow-hidden">
                        <DayPicker
                            ...
                            classNames={{
                                ...
                                selected:
                                    'bg-primary text-primary-foreground  hover:text-primary-foreground focus:bg-primary focus:text-primary-foreground rounded-l-md rounded-r-md',
...
const buildTime = ({
    ...
    const dateStr = format(value, formatStr)
        .replace(/(\d{4}-\d{2}-\d{2} )\d{2}/, `$1${pad(hour)}`)
        .replace(/:(\d{2}):/, `:${pad(minute)}:`)
        .replace(/:(\d{2})\./, `:${pad(second)}.`)
        .replace(/\s(AM|PM)/, ` ${ampm === AM_VALUE ? 'AM' : 'PM'}`);
